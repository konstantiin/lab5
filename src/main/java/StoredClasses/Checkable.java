        ...
        for (Field f: this.getClass().getDeclaredFields()){
            if (f.isAnnotationPresent(NotNull.class)){
                try {
                    ...
                    if (f.get(this) == null) throw new NullObjectException("Field \"" + f.getName() + "\" can't be null!");
            ...
            if (f.isAnnotationPresent(Boundaries.class)){
                ...
                try {
                    f.setAccessible(true);
                    if (l.compareTo(new BigDecimal(f.get(this).toString())) <= 0 && u.compareTo(new BigDecimal(f.get(this).toString())) >= 0) continue;
